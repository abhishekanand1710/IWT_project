...
function addLists(list, parentid, eid) {
    ...
    '<span class="text-light text">Total (INR)</span><strong class="text-light text">'+ list.length*revalPrice +'</strong></li> ';
...
function store(){
    ...
    for(i=0;i<rSubs.length;i++){
        firestore.collection("Results/CS/Revaluation").doc(usnv).collection('Subjects').add({
            code: rSubs[i].code,
            sub: rSubs[i].sub
        })
        .then(function(docRef) {
            ...
            if(i==rSubs.length  && done){
                ...
                alert(" Payment Successfully Completed!\n Revaluation application submitted. Thank You!")
            }
        })
        .catch(function(error) {
